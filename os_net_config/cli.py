...
def parse_opts(argv):
    ...
    parser.add_argument('--rollback', metavar='ROLLBACK',
                        help="""Its an internal argument and shall not be used """
                        """externally. During a migration from one provider."""
                        """to another, if any errors are seen, the previous """
                        """provider shall be restored.""",
                        choices=_PROVIDERS.keys(),
                        default=None)
