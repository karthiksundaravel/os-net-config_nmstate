    ...
    if re.match(r'[\w-]+$', ifname):
        ...
        for src in src_files:
            if os.path.exists(src):
                logger.info("%s: removing existing ifcfg script", ifname)
...
class IfcfgNetConfig(os_net_config.NetConfig):
    ...
    def ifcfg_get_dpdk_pci_address_list(self.ifcfg_dpdk_file):
        """
        Parse the ifcfg_dpdk_file and return a list of PCI addresses found in
        options:dpdk-devargs=... in the OVS_EXTRA line(s). The dpdk_
        """
        pci_addresses = []
    ...
    def destroy(self):
        ...
        for dpdk in self.del_device["dpdk"]:
            ...
            pci_addresses = self.ifcfg_get_dpdk_pci_address_list(ifcfg_dpdk)
