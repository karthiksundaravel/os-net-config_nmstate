...
After=systemd-udev-settle.service NetworkManager.service openvswitch.service network.target NetworkManager-dispatcher.service network.service
...
def unbind_dpdk_interfaces(pci_address):
    ...
    if iface_driver == "vfio-pci":
        err = common.unset_driverctl_override(pci_address)
    else:
...
def restore_dpdk_interfaces():
    ...
    for dpdk_nic in dpdk_map:
        ...
        if driver == dpdk_nic["driver"]:
            continue
        bind_dpdk_interfaces(pci_address, dpdk_nic["driver"])
...
def detach_dpdk_interfaces(pci_address):
    ...
    try:
        ...
        logger.error("%s: Failed to detach", pci_address)
        return -1
